...
def score_bullet(bullet: str) -> Dict[str, float]:
    ...
    has_metric = 1.0 if _has_metric(original) else 0.0
    has_tool = 1.0 if _contains_tool(original) else 0.0
    ownership = 1.0 if _has_ownership_signal(original) else 0.0
    impact = 1.0 if _has_impact_signal(original, has_metric=bool(has_metric)) else 0.0
    specificity = _specificity_score(words, bool(has_metric), bool(has_tool))
